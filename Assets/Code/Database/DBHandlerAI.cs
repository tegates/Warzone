...
public class DBHandlerAI 
{
	...
	{
		IDataReader goalReader = GameManager.Inst.DBManager.RunQuery(
		...
		while(goalReader.Read())
		{
			...
			IDataReader condReader = GameManager.Inst.DBManager.RunQuery(
	...
	{
		IDataReader actionReader = GameManager.Inst.DBManager.RunQuery(
			"SELECT goap_actions.* " +
		...
		while(actionReader.Read()) 
		{
			...
			//query for effects
			IDataReader effReader = GameManager.Inst.DBManager.RunQuery(
