...
namespace RootMotion.FinalIK {
	...
	public class IKSolverLookAt : IKSolver {
		...
		protected override void OnUpdate() {
			...
			if(_ikWeightState == -1)
			{
				IKPositionWeight = Mathf.Lerp(IKPositionWeight, 0, _ikWeightRate * Time.deltaTime);
			}
			else if(_ikWeightState == 1)
			{
				IKPositionWeight = Mathf.Lerp(IKPositionWeight, 1, _ikWeightRate * Time.deltaTime);
			}
			...
			{
				Vector3 tiltAxis = (spine[0].transform.up + Vector3.up * 2).normalized;
				Quaternion fixTilt = Quaternion.FromToRotation(head.transform.rotation * tiltAxis, tiltAxis);
				fixTilt = Quaternion.Slerp(Quaternion.identity, fixTilt, 1);

				head.transform.rotation = fixTilt * head.transform.rotation;
			}
		}
